...
export const authenticateApiKey = async (req, res, next) => {
  ...
  try {
    ...
    let creditBalance = 0;
    let monthlyAllowance = { limit: 0, used: 0, remaining: 0 };
    
    try {
      const { getCreditBalance, getMonthlyAllowance } = await import('../services/billing.js');
      creditBalance = await getCreditBalance(userSettings.user_id);
      monthlyAllowance = await getMonthlyAllowance(userSettings.user_id);
    } catch (error) {
      console.error('Failed to get credit info:', error);
      // Continue without credit info rather than failing auth
    }
...
export const apiRateLimit = (baseRequestsPerMinute = 60) => {
  ...
  return (req, res, next) => {
    ...
    const tierRateMultipliers = {
      'free': 1,
      'premium': 2,      // 120 RPM (as per plan)
      'premium_plus': 16.67, // ~1000 RPM (as per plan)
      'unlimited': Infinity,
      'enterprise': Infinity
    };
    
    const multiplier = tierRateMultipliers[userTier] || 1;
    const requestsPerMinute = multiplier === Infinity ? Infinity : Math.floor(baseRequestsPerMinute * multiplier);

    // Unlimited tiers bypass per-minute rate limiting
    ...
    if (currentCount >= requestsPerMinute) {
      return res.status(429).json({
        ...
        message: `Maximum ${requestsPerMinute} requests per minute allowed for ${userTier} tier`,
