...
export const createApiEmail = async (
  ...
  const { chargeCredits } = await import('./billing.js');
  ...
  // **CHARGE CREDITS BEFORE CREATING EMAIL**
  try {
    const chargeResult = await chargeCredits(userId, timeTier);
    console.log(`Credit charge successful: ${chargeResult.cost} credits from ${chargeResult.source}`);
  } catch (creditError) {
    if (creditError.message === 'Insufficient credits') {
      throw new Error('Insufficient credits. Please top up your account or upgrade your subscription.');
    }
    throw creditError;
  }
